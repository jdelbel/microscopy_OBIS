...
micro <- read_xlsx(here("files", "Calvert_phyto.xlsx"), sheet = "DFO2_4")
...
micro <- micro %>% 
  filter(!group == "Notes") %>%
  filter_at(vars(CPHY542:CPHY547), any_vars(!is.na(.)))
...
micro_tp <- micro_tp %>%
  ...
  rename(`Pennales` = `Unidentified pennate forms`)
```
...
metadata <- read_csv(here("files", "meta_phyto_2021-06-21.csv"))
...
micro_tp <- micro_tp %>% 
  select(date, collected, hakai_id, line_out_depth, work_area, site_id,
         ...
         `Chaetoceros ceratosporus`:Oligotrichea)
...
#Making data Tidy 
micro_tidy <- micro_tp %>% 
  pivot_longer(c(`Chaetoceros ceratosporus`: Oligotrichea),
...
micro_tidy <- micro_tidy %>% 
  mutate(group = case_when(species == "Biddulphiales" ~ "Bacillariophyta",
                            ...
                            species == "Cyanobacteria" ~ "Cyanobacteria",
                            TRUE ~ as.character (group)))
...
write_csv(micro_tidy, here("cleaned_csv", "dfo2_invoice243-244_2.csv"))
```
