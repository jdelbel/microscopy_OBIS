...
micro <- read.csv(here("files", "toba_2022-04-07.csv"), na.strings = c("","NA"))
...
```{r}
#Dealing with analyst notes -  removed notes for this upload.
...
micro <- micro %>% 
  filter(!group == "Notes") %>% 
  filter_at(vars(QPHY1462:QPHY1494), any_vars(!is.na(.)))
...
metadata <- read_csv(here("files", "toba_meta.csv"))
...
micro_tp <- micro_tp %>% 
  select(date, collected, hakai_id, line_out_depth, work_area, site_id,
         ...
         `Asterionellopsis glacialis`:Oligotrichea)
...
#Making data Tidy 
micro_tidy <- micro_tp %>% 
  pivot_longer(c(`Asterionellopsis glacialis`: Oligotrichea),
...
write_csv(micro_tidy, here("cleaned_csv", "microscopy_toba_2022-04-07.csv"))
