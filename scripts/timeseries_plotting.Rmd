...
```{r}
micro_diat <- micro %>% 
  filter(group == "Bacillariophyta" & count > 20000 & !is.na(genus)) %>% 
  group_by(date, genus) %>% 
  summarise(count_sum = sum(count)) %>% 
  ungroup() %>% 
  complete(date, genus) %>% 
...
p1 <- micro_diat %>% 
 filter(date > "2016-01-01" &
  ...
  scale_x_date(limits = startEnd_1,
               ...
               labels = scales::date_format("%m")) +
  scale_fill_brewer(palette = "Spectral",
                      name = "Genus, > 20000 cells") +
  theme_bw() +
  labs(y = bquote("Abundance (cells" ~ L^-1*")")) +
  theme(legend.position = c(0.13, 0.5),
        legend.background =  element_blank(),
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.title.y = element_blank(),
...
p4 <- micro_diat %>% 
 filter(date > "2019-01-01" &
  ...
  theme_bw() +
  labs(y = bquote("Abundance (cells" ~ L^-1*")")) +
  theme(legend.position = c(0.085, 0.5),
        legend.background =  element_blank(),
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.title.y = element_blank(),
...
ggsave(here("figures", "qu39_ts_diatom_test.png"), fig_diat,
...
micro %>% 
 filter(group == "Chlorophyta-Prasinophyta" &
           date > "2016-01-01" &
           date < "2016-12-31") %>%
  complete(date, group, scientificName_accepted) %>% 
