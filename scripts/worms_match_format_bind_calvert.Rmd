...
#2020 data.
micro_1 <- read_csv(here("cleaned_csv", "microscopy_dfo2_2021-05-03.csv"))

#this is exactly the same as the micro_1 - 202 data
micro_2 <- read_csv(here("cleaned_csv", "microscopy_kc10_2021-05-03.csv"))

#This only goes to 2019-07-24
micro_3 <- read_csv(here("cleaned_csv", "microscopy_qcs01_2021-05-03.csv"))
...
worm_match_clean <- worm_match %>% 
  filter(!(str_detect(orig_name, "Pleurostomum gracile") |  
           ...
           str_detect(orig_name, "Jacobia iberia"))) %>% 
...
complete <- complete %>% 
  ...
  filter(!(str_detect(orig_name, "Pleurostomum gracile") |  
           str_detect(orig_name, "Phyllomitus yorkeensis") |  
           str_detect(orig_name, "Meringosphaerae tenerrima") |  
           str_detect(orig_name, "Jakobia iberia"))) 
...
```
```{r}
#With Calvert data, quite a few mistakes and species with no match.
#Lets see where:

mistake_1 <- complete_join %>% 
  filter(orig_name == "Phaeocystis pouchetii non flagella")
#DFO 2 - 2019 - 2020 - This can likely be dealt with once the data is bound (fixed in bound sheet)

mistake_2 <- complete_join %>% 
  filter(orig_name == "Radiolaria")
#DFO2 and KC10 - Should be changed to Radiozoa (fixed in bound sheet) - Might have been better to just fix in here... ridiculous the amount of back and forth.

mistake_3 <- complete_join %>% 
  filter(orig_name == "Raphidiophyta")
#KC10 - 2019-01-01 - change to Raphidophyceae (can fix in bound sheet)

mistake_4 <- complete_join %>% 
  filter(orig_name == "Unidentified centric forms")
#QCS01 - 2018 - remove from original sheet (should troubleshoot this at source - was in _3, should be fixed)

mistake_5 <- complete_join %>% 
  filter(orig_name == "Unknown species")
#KC10 - 2020 - remove from original sheet - Issue - there is a count under rapidophyta group row. Fixed and put count in species row.

#going to try to fix these once the data is bound....
...
#For whatever reason, this wasn't removed previously. So doing it here, but need to troubleshoot when submitting to OBIS.
complete_join <- complete_join %>% 
  filter(!orig_name == "Jacobia iberia")

complete_join <- complete_join %>% 
  mutate(group = case_when(orig_name == "Chaetoceros" & is.na(group) ~ "Bacillariophyta",
                           orig_name == "Cyanobacteria" & is.na(group) ~ "Cyanobacteria",
                           TRUE ~ as.character(group)))

complete_join <- complete_join %>% 
  mutate(group = case_when(orig_name == "Protozoa" & 
                           group == "Unknown very small flagellated species" ~ 
                           "Unknown flagellae-bearing cells",
                           TRUE ~ as.character(group)))

#Unknown flagellae-bearing cells and Unknown very small flagellated species need to be combined. I will fix here for now

complete_distinct <-  complete_join %>% 
  distinct(valid_name, orig_name, group, lifeStage, 
           identificationQualifier, identificationRemark)
```
...
write_csv(complete_join, here("output_worms_matching", "calvert_2018_2020.csv"))
```
