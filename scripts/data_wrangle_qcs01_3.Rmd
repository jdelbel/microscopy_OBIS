...
micro <- read_xlsx(here("files", "Calvert_phyto.xlsx"), sheet = "QCS01_3")
...
micro <- micro %>% 
  filter(!group == "Notes") %>%
  filter_at(vars(CPHY404:CPHY468), any_vars(!is.na(.)))
...
micro_tp <- micro_tp %>% 
  ...
  mutate(#Biddulphiales = sum(`Unidentified centric forms`, 
         ...
         Oligotrichea = sum(`Oligotricious forms (large)`,
...
sum_check <- micro_tp %>% 
  select(#Biddulphiales, `Unidentified centric forms`, 
         ...
         Oligotrichea, `Oligotricious forms (large)`,
...
write_csv(micro_tidy, here("cleaned_csv", "qcs01_3.csv"))
```
