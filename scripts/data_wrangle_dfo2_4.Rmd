...
micro <- read_xlsx(here("files", "dfo2.xlsx"), sheet = "dfo2_4")
...
micro <- micro %>% 
  filter(!group == "Notes") %>%
  filter_at(vars(CPHY403:CPHY470), any_vars(!is.na(.)))
...
micro_tp <- micro_tp %>%
  rename(`Chaetoceros spp.` = `Chaetoceros spp_sum.`,
         Cyanobacteria = `Cyanobacteria (~large cells)`)
```
...
write_csv(micro_tidy, here("cleaned_csv", "dfo2_4.csv"))
```
