...
micro <- read_xlsx(here("files", "Calvert_phyto.xlsx"), sheet = "KC10_1")
...
#Some sheets have chrysochromulina as a euglenophyte. It's a hapto.
micro <- micro %>% 
...
micro <- micro %>% 
  filter(!group == "Notes") %>%
  filter_at(vars(CPHY412:CPHY518), any_vars(!is.na(.)))
...
micro <- micro %>% 
  mutate(species = str_replace(species, "Chatonella/Olisthodiscus", "Olisthodiscus"))
...
write_csv(micro_tidy, here("cleaned_csv", "kc10_1.csv"))
```
