...
micro <- read_xlsx(here("files", "dfo2.xlsx"), sheet = "dfo2_1")
...
#The id should be the hakai_id... Didn't I have this?
...
micro <- micro %>% 
  filter(!group == "Notes") %>%
  filter_at(vars(CPHY528:CPHY533), any_vars(!is.na(.)))
...
micro_tp <- micro_tp %>% 
  select(date, collected, hakai_id, line_out_depth, work_area, site_id,
         ...
         `Chaetoceros cinctus`:Oligotrichea)
...
#Making data Tidy 
micro_tidy <- micro_tp %>% 
  pivot_longer(c(`Chaetoceros cinctus`: Oligotrichea),
...
write_csv(micro_tidy, here("cleaned_csv", "dfo2_1.csv"))
```
