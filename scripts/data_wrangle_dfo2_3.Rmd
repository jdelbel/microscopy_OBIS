...
micro <- read_xlsx(here("files", "dfo2.xlsx"), sheet = "dfo2_3")
...
notes <- notes %>%
  ...
  unite("analyst_note", Notes_1:Notes_6, sep = " ", na.rm = TRUE)
...
micro <- micro %>% 
  filter(!group == "Notes") %>%
  filter_at(vars(CPHY497), any_vars(!is.na(.)))
...
#No extra column containing information.
```
...
#Can't be bothered to deal with why my transpose eliminated the hakai ID when I'm uploading a single sample. Super annoying. Just re-entering here as I very rarely do this with a single sample.
micro_tp <- micro_tp %>% 
  mutate(hakai_id = case_when(hakai_id == 1 ~ "CPHY497"))
...
sum_check <- micro_tp %>% 
  select(#Biddulphiales, 
         #`Unidentified centric forms`,
...
micro_tp <- micro_tp %>% 
  select(#-`Unidentified centric forms`,
         #-`Unidentified very small centric forms`,
         -`Unidentified pennate forms`,
         -`Unidentified very small pennate forms`,
         -`S. marinoi (small cells)`, 
...
micro_tp <- micro_tp %>%
  rename(Biddulphiales = `Unidentified centric forms`,
         `Chaetoceros tenuissimus` = `Chaetoceros tenuissimus (small)`) 
```
...
write_csv(micro_tidy, here("cleaned_csv", "dfo2_3.csv"))
```
