...
micro <- read.csv(here("files", "qu39_invoice_245-2_2019-data.csv"), 
                  na.strings = c("","NA"))
...
```{r}
#List of Louis group names to associate with each species (right now group names in species list above species names ) - list needs to be updated if there are differences or new groups added - hopefully encompassing eventually, but use caution as Louis makes small changes regularily.
...
micro <- micro %>% 
  filter(!group == "Notes") %>% 
  filter_at(vars(QPHY1227:QPHY630), any_vars(!is.na(.)))
...
write_csv(micro_tidy, here("cleaned_csv", "microscopy_qu39_2019_invoice245_add.csv"))
