...
micro <- read_xlsx(here("files", "Calvert_phyto.xlsx"), sheet = "QCS01_1")
...
#Notes are a mess for this sheet, so disregard sheet!
...
micro <- micro %>% 
  filter(!group == "Notes") %>%
  filter_at(vars(CPHY476:CPHY516), any_vars(!is.na(.)))
...
# micro_tp <- micro_tp %>% 
#   rename(`Cheatoceros spp.` = `Chaetoceros spp_sum.`)
```
...
write_csv(micro_tidy, here("cleaned_csv", "qcs01_1.csv"))
```
