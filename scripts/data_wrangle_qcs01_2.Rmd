...
micro <- read_xlsx(here("files", "Calvert_phyto.xlsx"), sheet = "QCS01_2")
...
micro <- micro %>% 
  filter(!group == "Notes") %>%
  filter_at(vars(CPHY527:CPHY530), any_vars(!is.na(.)))
...
#`Elongate form` Wasn't in the list but need to ensure added back in when present!
...
write_csv(micro_tidy, here("cleaned_csv", "qcs01_2.csv"))
```
